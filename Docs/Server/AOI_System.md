...
│  │                                                         │     │
│  │  GetVisibleEntities(clientId) → HashSet<uint>          │     │
│  └────────────────────────────────────────────────────────┘     │
│                             │                                   │
└─────────────────────────────┼───────────────────────────────────┘
                              ▼
...
- `Assets/Scripts/Network/NetAdapter/AOI/SpatialHashGrid.cs` (150 lines)
...
foreach (var entityId in _queryBuffer)
{
    ...
    Vector3 entityPos = /* extract from entity type */;
...
**Called from:** `ServerGameLoop.cs:370` during snapshot broadcast
...
**Client action:**
- Stop rendering entity
- Destroy GameObject (or pool for reuse)
- Remove from interpolation buffer
...
- **[Server_Loop.md](Server_Loop.md)** - Server tick loop and snapshot broadcasting
